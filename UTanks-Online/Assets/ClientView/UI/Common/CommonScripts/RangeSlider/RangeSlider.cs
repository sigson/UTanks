...
namespace UnityEngine.UI.Extensions
{
    ...
    public class RangeSlider : Selectable, IDragHandler, IInitializePotentialDragHandler, ICanvasElement
    {
        ...
        [SerializeField]
        private float m_MinimalHandleToHandleValue = 0;

        public float MinimalHandleToHandleValue
        {
            get { return m_MinimalHandleToHandleValue; }
            set
            {
                SetLow(m_MinValue); SetHigh(m_MaxValue);
        ...
        protected override void OnValidate()
        {
            ...
            if (WholeNumbers)
            {
                ...
                m_MinimalHandleToHandleValue = Mathf.Round(m_MinimalHandleToHandleValue);
        ...
        {
            // Clamp the input
            float newValue = Mathf.Clamp(input, MinValue, HighValue - MinimalHandleToHandleValue); //clamp between min and High
            if (WholeNumbers)
            {
        ...
        {
            // Clamp the input
            float newValue = Mathf.Clamp(input, LowValue + MinimalHandleToHandleValue, MaxValue); //clamp between min and High
            if (WholeNumbers)
            {
                newValue = Mathf.Round(newValue);
            }
