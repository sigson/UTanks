...
namespace SecuredSpace.AudioManager.Logger {
    /// <summary>
    ...
    public class LoggedAudioManager : IAudioManager {
        ...
        public AudioError PlayBlock(List<string> names, ChildType child = ChildType.PARENT)
        {
            const string enterLogBase = "Attempting to play the registered AudioSource entry";
            const string exitLogBase = "Playing registered AudioSource entry";
            ...
            {
                OnMethodEnter(enterLogBase, name);
                error = AudioHelper.ConvertToAudioError(m_wrappedInstance?.PlayWithSettings(name, timeOffset, child));
                OnReceivedError(exitLogBase, error);
                OnMethodExit(exitLogBase, error);
            }
        ...
        public AudioError PlayWithSettings(string name, float timeOffset, ChildType child = ChildType.PARENT)
        {
            const string enterLogBase = "Attempting to play the registered AudioSource entry";
            const string exitLogBase = "Playing registered AudioSource entry";

            OnMethodEnter(enterLogBase, name);
            AudioError error = AudioHelper.ConvertToAudioError(m_wrappedInstance?.PlayWithSettings(name, timeOffset, child));
            OnReceivedError(exitLogBase, error);
            OnMethodExit(exitLogBase, error);
            return error;
        }
