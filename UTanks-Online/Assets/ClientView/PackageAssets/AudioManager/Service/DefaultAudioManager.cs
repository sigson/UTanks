...
namespace SecuredSpace.AudioManager.Service {
    public class DefaultAudioManager : IAudioManager {
        ...
        public AudioAnchor audioAnchor = null;
        ...
        {
            m_soundDictionary.ForEach(x =>
            {
                x.Value.DestroyCleanup();
                if (m_parentBehaviour != null)
                    m_parentBehaviour.StopAllCoroutines();
                if(this.audioAnchor != null)
                    this.audioAnchor.StopAllCoroutines();
            });
        }
        ...
        public void UpdateSettings()
        {
            if (audioAnchor != null)
            {
                m_soundDictionary.ForEach(x =>
                {
                    x.Value.Volume = audioAnchor.volume;
                });
            }
        }
        ...
        public AudioError PlayBlock(List<string> names, ChildType child)
        {
            float timeOffset = 0f;
            AudioError error = default;
            foreach (var name in names)
            {
                error = TryGetSource(name, out var source);
                
                if (error != AudioError.OK)
                {
                    return error;
                }
                error = this.PlayWithSettings(name, timeOffset, child);
                timeOffset += AudioHelper.GetLengthOfClip(source);
            }
            return error;
        }

        public AudioError PlayWithSettings(string name, float timeOffset, ChildType child)
        {
            AudioError error = TryGetSource(name, out var source);
            ...
            UpdateSettings();
            if(source.IsPlaying)
                source.InvokeChild(child, (s) => s.Stop());
            error = SetStartTime(name, AudioHelper.GetStartOfClip(source), child);
            error = source.InvokeChild(child, (s) => s.PlayDelayed(timeOffset));
            return error;
        }
        ...
        public string[] GetNowPlayingAudioName()
        {
            var result = new List<string>();
            m_soundDictionary.ForEach(x =>
            {
                if(x.Value.Source.isPlaying && !x.Value.inFade)
                {
                    result.Add(x.Key);
                }
            });
        ...
        public AudioError SoftStop(string name, bool unloop = false, ChildType child = ChildType.PARENT)
        {
            AudioError error = TryGetSource(name, out var source);
            ...
            //source.Loop = false;
            error = source.InvokeChild(child, (s) => s.Stop());
            var weaponEndStart = source.AudioSourceSettingCache.audioClip.length - (source.AudioSourceSettingCache.audioClip.length * 0.05f);
            source.Time = (weaponEndStart);
            error = source.InvokeChild(child, (s) => s.Play());
            source.Loop = false;
            return error;
        }

        public AudioError SoftStopAll(bool unloop = false, ChildType child = ChildType.PARENT)
        {
            m_soundDictionary.ForEach(x =>
            {
                if(x.Value.IsPlaying)
                    SoftStop(x.Key, unloop, child);
            });
            return AudioError.OK;
        }

        public AudioError Reset(string name, ChildType child = ChildType.PARENT)
        {
            AudioError error = TryGetSource(name, out var source);
        ...
        public AudioError AddSoundFromAudioSourceSettings(AudioSourceSetting audioSourceSetting)
        {
            throw new System.NotImplementedException();
        }

        public AudioError StopAll(ChildType child = ChildType.PARENT)
        {
            m_soundDictionary.ForEach(x =>
            {
                Stop(x.Key, child);
        ...
        public AudioError Fade(string name, ChildType child = ChildType.PARENT)
        {
            AudioError error = TryGetSource(name, out var source);
        ...
        public AudioError FadeAll(List<string> excludeList = null, ChildType child = ChildType.PARENT)
        {
            m_soundDictionary.ForEach(x =>
            {
                if(excludeList == null || !excludeList.Contains(x.Key))
                    Fade(x.Key, child);
            });
            return AudioError.OK;
        }
