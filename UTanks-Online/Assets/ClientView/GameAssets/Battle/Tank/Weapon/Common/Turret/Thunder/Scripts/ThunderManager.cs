...
{
    public class ThunderManager : ITurret
    {
        ...
        public override void Initialize(TankManager tankManager, ECSEntity player)
        {
            ...
            var weaponComponent = player.GetComponent<ThunderDamageComponent>();
            ...
            explosiveSprite.GetComponent<AnimationScript>().frames = tankManager.TurretResources.GetElement<ItemCard>("fire_anim").GetListOfItems<Sprite>();

            weaponAudio.audioStorage.Add(tankManager.TurretResources.GetElement<AudioSourceSetting>("thunder_shot").UpdateSettings("thunder_shot", false, 1f));
            weaponAudio.Build();
        ...
        public override void TurretInfluenceEvent(ECSEntity player, ECSEvent updatingEvent)
        {
            ...
            if (updatingEvent.GetId() == ShotEvent.Id)
            {
                try
                {
                    var smokyDamage = player.GetComponent<ThunderDamageComponent>();
                    ...
                    if (weaponAudio != null)
                        weaponAudio.audioManager.PlayBlock(new List<string> { "thunder_shot" });
                }
                catch
                {
        ...
        public override void InputOperations()
        {
            ...
            if (ShootTurn > 0 && (!parentTankManager.Ghost && !parentTankManager.checkGhost))
            {
                ...
                if ((playerEntity.GetComponent<WeaponEnergyComponent>().MaxEnergy == playerEntity.GetComponent<WeaponEnergyComponent>().Energy) && !playerEntity.HasComponent(WeaponTechCooldownComponent.Id))
                {
                    ...
                    var smokyDamage = playerEntity.GetComponent<ThunderDamageComponent>();
        ...
        {
            return turretObject.AddComponent<ThunderManager>();
        }
    }
